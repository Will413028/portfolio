  ...
  return (
    <section className="px-6 py-24 max-w-6xl mx-auto">
      <div className="text-center mb-16">
        <p className="text-[11px] uppercase tracking-[0.3em] text-zinc-500 mb-4">Case Studies</p>
        <h2 className="text-4xl md:text-5xl font-medium">
          Curated <span className="font-serif italic gradient-text-pink">work</span>
        </h2>
      </div>
          ...
          return (
            <div key={project.id} className="flex flex-col">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[11px] text-zinc-500">{project.quarter}</span>
              </div>
              ...
              <Link href={`/work/${project.slug}`} className="group block mb-3">
                <h3 className="text-xl font-serif text-white group-hover:text-zinc-300 transition-colors">
                  {project.title}
                  {project.subtitle && (
                    <span className="text-zinc-500 ml-2 text-base font-serif">{project.subtitle}</span>
                  )}
                </h3>
              </Link>
